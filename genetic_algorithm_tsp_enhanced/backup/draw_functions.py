...
def draw_plot(screen: pygame.Surface, x: list, y: list, x_label: str = 'Generation', y_label: str = 'Fitness') -> None:
    ...
    # Attach Agg canvas to figure
    canvas = FigureCanvasAgg(fig)
    canvas.draw()

    # Get width/height and RGBA buffer
    width, height = canvas.get_width_height()
    buf = canvas.buffer_rgba()  # returns a Python buffer / memoryview

    # Create a Pygame surface from RGBA buffer
    # frombuffer expects a bytes-like object; memoryview is fine
    surf = pygame.image.frombuffer(buf, (width, height), "RGBA")

    # Optional convert for faster blitting on some displays
