...
class HospitalDeliverySystem:
    ...
    def create_deliveries(self):
        """Cria lista de entregas com prioridades e pesos (determinístico)"""
        self.deliveries = []
        self.priorities = {}
        ...
        # Usar seed fixa para garantir mesma ordem sempre
        rng = random.Random(42)  # Seed fixa = resultados reproduzíveis
        rng.shuffle(priorities_list)
        
        # Listas de tipos de itens por prioridade
        critical_items = [
        ...
        for i, location in enumerate(self.delivery_locations):
            ...
            # Usar seed baseada no índice para garantir mesmo peso sempre
            if priority == 1:  # Crítico
                weight = 2 + (i * 13 % 13)  # Peso entre 2 e 15
                item_type = critical_items[i % len(critical_items)]
            elif priority == 2:  # Urgente
                weight = 5 + (i * 25 % 25)  # Peso entre 5 e 30
                item_type = urgent_items[i % len(urgent_items)]
            else:  # Normal
                weight = 10 + (i * 40 % 40)  # Peso entre 10 e 50
                item_type = normal_items[i % len(normal_items)]
